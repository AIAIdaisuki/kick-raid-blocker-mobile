...
本 Userscript は **ブラウザ内でのみ動作** します。Kick の公式アプリ（ネイティブアプリ）には届きません。アプリのまま守りたい人向けに、すぐ使える設定ファイルを用意しています：

| パス | コスト | レイドブロック | チャット維持 | アーティファクト |
| --- | --- | --- | --- | --- |
| **Pathα: NextDNS で Pusher 遮断** | 無料 | ✅ | ❌ real-time 死 | [`proxy/nextdns-denylist.txt`](proxy/nextdns-denylist.txt) |
| **Pathβ: Surge + WebSocket スクリプト** | $49.99 | ✅ | ✅ | [`proxy/kick-raid-blocker.sgmodule`](proxy/kick-raid-blocker.sgmodule) |
| **Pathβ': Loon (3.x+)** | $4.99 | ✅ | ✅ | [`proxy/kick-raid-blocker.plugin`](proxy/kick-raid-blocker.plugin) |

セットアップ全文 → [**docs/app-raid-blocking.md**](docs/app-raid-blocking.md)
