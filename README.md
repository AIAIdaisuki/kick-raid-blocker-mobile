...
本 Userscript は **ブラウザ内でのみ動作** します。Kick の公式アプリ（ネイティブアプリ）には届きません。アプリのまま守りたい人向けに、複数のパスを用意しています：

| パス | コスト | レイドブロック | チャット維持 | 確実性 |
| --- | --- | --- | --- | --- |
| **γ: 自分の VPS + mitmproxy** ⭐ | **既存VPSなら 0円** | ✅ | ✅ | ✅ 検証済み |
| **α: NextDNS で Pusher 遮断** | 無料 | ✅ | ❌ real-time 死 | ✅ |
| **β: Surge / Loon プラグイン** | $5〜$50 | ⚠ | ✅ | ❌ 未検証 |

**VPSをお持ちなら γ が最善** です。コスト0でアプリ・チャット・出先全部維持できます。

VPSセットアップ全文 → [**docs/vps-setup.md**](docs/vps-setup.md)
全パス比較 → [**docs/app-raid-blocking.md**](docs/app-raid-blocking.md)
