...
本 Userscript は **ブラウザ内でのみ動作** します（iPhone Safari / Android Chrome / PC ブラウザ）。Kick の公式アプリ（App Store / Play Store のネイティブアプリ）の中では動きません。

**最も簡単な解決策**は **Safari で `kick.com` を開いて「ホーム画面に追加」** することで、アプリ風の見た目で Userscript が動く環境が手に入ります（コスト 0・5分）。

詳しくは [**docs/app-raid-blocking.md**](docs/app-raid-blocking.md) で 3 つの実装方法（PWA / NextDNS / 有料プロキシ）を比較しています。
